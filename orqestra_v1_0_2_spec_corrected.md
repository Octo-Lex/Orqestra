...
### 5.2.1 Mandatory Stronghold Compatibility Preflight

Before implementing the Stronghold-backed vault, verify that the current `tauri-plugin-stronghold` release is compatible with the installed Tauri 2 version and the transitive `iota-stronghold` API used by the plugin.

Required preflight commands:

```powershell
cargo search tauri-plugin-stronghold
cargo search iota-stronghold
cargo tree -p orqestra-desktop | Select-String -Pattern "tauri|stronghold"
```

Equivalent Unix shell form:

```bash
cargo search tauri-plugin-stronghold
cargo search iota-stronghold
cargo tree -p orqestra-desktop | grep -E "tauri|stronghold"
```

Decision rule:

| Finding | Required implementation path |
|---|---|
| `tauri-plugin-stronghold` is compatible with the installed Tauri 2 version | Implement `SecretVault` with Stronghold for vault storage and OS keychain delegation for the vault unlock secret |
| `tauri-plugin-stronghold` lags the installed Tauri 2 version or has incompatible `iota-stronghold` APIs | Implement `SecretVault` directly with the `keyring` crate as the persisted secret backend, document the substitution, and keep the command-layer API unchanged |
| OS keychain backend is unavailable on the target machine | Use explicit `session-only` mode or `unavailable`; do not silently downgrade to plaintext, XOR, or renderer-side persistence |

The `SecretVault` trait is the release contract. Stronghold is the preferred implementation, but a keyring-backed implementation is acceptable for v1.0.2 if the Stronghold plugin compatibility preflight fails. This substitution must be recorded in README, CHANGELOG, and release notes under the security implementation details.
...
- Stronghold compatibility is checked before implementation and the selected backend is documented.
- Preferred path: GitHub PATs are stored in Stronghold, not XOR vault or plaintext JSON.
- Fallback path: if Stronghold compatibility fails, GitHub PATs are stored through a `keyring`-backed `SecretVault` implementation, not XOR vault or plaintext JSON.
- The Stronghold unlock secret, when Stronghold is used, is stored in the OS keychain.
- On Windows, the unlock secret or keyring entry is stored through Windows Credential Manager.
...
- Deploy to Cloudflare Pages only on `master`, and skip the deploy step cleanly when `CLOUDFLARE_API_TOKEN` or related configuration is absent. Do not compare `secrets.*` directly in a job-level `if:` condition.
      ...
      - name: Check Cloudflare deployment configuration
        ...
        run: |
          if [ -z "$CLOUDFLARE_API_TOKEN" ] || [ -z "$CLOUDFLARE_ACCOUNT_ID" ]; then
            ...
            echo "::notice::Cloudflare credentials are not configured; skipping Pages deployment."
          else
            echo "deploy=true" >> "$GITHUB_OUTPUT"
          fi
      - name: Deploy Cloudflare Pages
        if: steps.cloudflare-config.outputs.deploy == 'true'
...
- Workflow does not use `secrets.*` comparisons in job-level `if:` conditions.
...
- Windows x64.
- macOS universal (`universal-apple-darwin`) by default; if universal packaging fails, produce clearly labeled separate `x86_64` and `arm64` artifacts instead.
- Linux x64.
      ...
      matrix:
        include:
          - os: windows-latest
            ...
            tauri-target: ""
          - os: macos-latest
            artifact-name: orqestra-macos-universal
...
### 7.4.1 macOS Artifact Rule

The macOS release artifact must not accidentally ship as an unlabeled Apple-Silicon-only build. Because `macos-latest` can produce an arm64 build by default, the workflow must explicitly request a universal binary when a single macOS artifact is expected:

```yaml
- run: rustup target add x86_64-apple-darwin aarch64-apple-darwin
- run: npm run tauri build -w apps/desktop -- --target universal-apple-darwin
```

If universal packaging fails or materially complicates the release, v1.0.2 may ship separate macOS artifacts instead, but they must be clearly labeled:

```text
orqestra-macos-arm64
orqestra-macos-x86_64
```

An unlabeled arm64-only macOS artifact does not satisfy the v1.0.2 cross-platform release requirement.
...
- macOS artifact is universal, or separate macOS arm64/x86_64 artifacts are clearly labeled.
- No unlabeled arm64-only macOS artifact is published as the generic macOS download.
