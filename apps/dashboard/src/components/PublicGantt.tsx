...
import { STATUS_COLORS, type Task } from '../lib/data';
...
export const PublicGantt: React.FC<{ tasks: Task[] }> = ({ tasks }) => {
  ...
  useEffect(() => {
    ...
    const allStarts = dated.map(t => parseDate(t.start_date!));
    const allEnds = dated.map(t => parseDate(t.due_date!));
    ...
    const height = HEADER_HEIGHT + dated.length * ROW_HEIGHT + 20;
