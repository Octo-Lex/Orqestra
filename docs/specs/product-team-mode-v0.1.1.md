...
**Scope:** Defines how Orqestra will support governed multi-agent product team workflows  
...
|---------|-------|-----------------|
| **v2.16.0** | Product Team Mode Foundations | Spec committed, directory/persistence policy finalized, typed schemas only, role permission declarations only, event type definitions only. No agent execution. |
| **v2.17.0** | Intake + Project Knowledge Pack | Repo scan, domain-language draft, architecture-map draft, stack-map draft, human approval gate. No patch proposals. |
| **v2.18.0** | Plan + PRD + Issue Graph | Multi-role planning, PRD draft with approval gate, issue-graph decomposition. No source patching. |
| **v2.19.0** | Role-Bounded Patch Proposals | Implementation agent proposal-only, role-aware patch guard. No commit, no merge. |
| **v2.20.0** | QA/Evidence/Dashboard Surface | QA plan/report, event-sourced evidence derivation, dashboard feature progress panel. |
...
## 12. Acceptance Bar for MVP (v2.16.0–v2.17.0)
